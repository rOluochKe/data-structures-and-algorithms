...
public class EcosystemSimulationWithGenderAndStrength {
    ...
    for (int i = 0; i < ARRAY_SIZE; i++) {
      for (int j = 0; j < ARRAY_SIZE; j++) {
        ...
        if (rand < BEAR_CREATION_PROBABILITY) {
          river[i][j] = new Bear(random.nextBoolean(), random.nextDouble());
        } else if (rand < FISH_CREATION_PROBABILITY) {
          river[i][j] = new Fish(random.nextBoolean(), random.nextDouble());
        }
      }
    }
